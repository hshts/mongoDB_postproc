...
class Testgetcoordination(unittest.TestCase):
    ...
    def test_Li(self):
        self.assertEqual(get_mode_specie_sites(
            getcoordination(Structure.from_file('test_structures/Li_mp-135_conventional_standard.cif'))), {'Li': 8})

    def test_Mg(self):
        self.assertEqual(get_mode_specie_sites(
            getcoordination(Structure.from_file('test_structures/Mg_mp-153_conventional_standard.cif'))), {'Mg': 12})

    def test_Zn(self):
        self.assertEqual(get_mode_specie_sites(
            getcoordination(Structure.from_file('test_structures/Zn_mp-79_conventional_standard.cif'))), {'Zn': 6})
    ...
    def test_ZnSb(self):
        self.assertEqual(get_mode_specie_sites(
            getcoordination(Structure.from_file('test_structures/ZnSb_mp-753_conventional_standard.cif'))),
                         {'Zn': 4, 'Sb': 4})

    def test_ZnS(self):
        self.assertEqual(get_mode_specie_sites(
            getcoordination(Structure.from_file('test_structures/ZnS_mp-10695_conventional_standard.cif'))),
                         {'Zn': 4, 'S': 4})
